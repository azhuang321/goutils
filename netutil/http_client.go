//reference: https://github.com/liuyongshuai/goUtils/blob/master/http_client.go
...
func NewEmptyHttpClient() *HttpClient {
	ret := &HttpClient{
		...
		traceId:     strutil.RandomChars(32),
	}
	return ret
}
...
func (ehc *HttpClient) AddFile(fieldName, filePath, fileName string) *HttpClient {
	if !fsutil.FileExist(filePath) {
		fmt.Printf("_HttpClient_AddFile_error||fieldName=%v||filePath=%v||fileName=%v||file not exists", fieldName, filePath, fileName)
		return ehc
	}
	if len(fileName) <= 0 {
...
func (ehc *HttpClient) Get() (*HttpResponse, error) {
	...
	if err != nil {
		fmt.Printf("_HttpClient_Get_error||%v||err=%v||http.NewRequest failed", ehc.getComErrMsg(), err)
		return NewHttpResponse(nil), err
	}
	response, err := ehc.do(httpReq)
	if err != nil {
		fmt.Printf("_HttpClient_Get_failure||%v||err=%v", ehc.getComErrMsg(), err)
		return NewHttpResponse(nil), err
	}
...
func (ehc *HttpClient) Head() (*HttpResponse, error) {
	...
	if err != nil {
		fmt.Printf("_HttpClient_Head_error||%v||err=%v||http.NewRequest failed", ehc.getComErrMsg(), err)
		return NewHttpResponse(nil), err
	}
	resp, err := ehc.do(httpReq)
	if err != nil {
		fmt.Printf("_HttpClient_Head_failure||%v||head http request failed|||err=%v", ehc.getComErrMsg(), err)
		return NewHttpResponse(nil), err
	}
...
func (ehc *HttpClient) Post() (*HttpResponse, error) {
	...
	if err != nil {
		fmt.Printf("_HttpClient_Post_error||%v||err=%v||http.NewRequest failed", ehc.getComErrMsg(), err)
		return NewHttpResponse(nil), err
	}
	...
	resp, err := ehc.do(httpReq)
	if err != nil {
		fmt.Printf("_HttpClient_Post_failure||%v||err=%v", ehc.getComErrMsg(), err)
		return NewHttpResponse(nil), err
	}
	return ehc.processResponse(resp, err)
}
...
func (ehc *HttpClient) procWriter() *multipart.Writer {
	...
	for _, f := range ehc.uploadFiles {
		...
		if err != nil {
			fmt.Printf("_HttpClient_procWriter_error||%v||file=%v||err=%v||CreateFormFile failed", commErrMsg, f.FilePath, err)
			continue
		}
		srcFile, err := os.Open(f.FilePath)
		if err != nil {
			fmt.Printf("_HttpClient_procWriter_error||%v||err=%v||file=%v||open file failed", commErrMsg, err, f.FilePath)
			continue
		}
		_, err = io.Copy(formFile, srcFile)
		if err != nil {
			fmt.Printf("_HttpClient_procWriter_error||%v||err=%v||file=%v||copy file failed", commErrMsg, err, f.FilePath)
		}
		err = srcFile.Close()
		if err != nil {
			fmt.Printf("_HttpClient_procWriter_error||%v||err=%v||file=%v||close file failed", commErrMsg, err, f.FilePath)
		}
	}
	//写入POST的值
	for k, vs := range ehc.vals {
		...
		if err != nil {
			fmt.Printf("_HttpClient_procWriter_error||%v||err=%v||key=%v||values=%+v||WriteField failed", commErrMsg, err, k, vs)
		}
	}
	err := writer.Close()
	if err != nil {
		fmt.Printf("_HttpClient_procWriter_error||%v||err=%v||Writer Close failed", commErrMsg, err)
	}
	return writer
}
...
func (ehc *HttpClient) processResponse(response *http.Response, err error) (*HttpResponse, error) {
	...
	if err != nil {
		ret.err = err
		fmt.Printf("_HttpClient_processResponse_error||%v||err=%v", commErrMsg, err)
		return ret, err
	}
	if response == nil {
	...
	defer func() {
		...
		if err != nil {
			fmt.Printf("_HttpClient_processResponse_failure||%v||err=%v||response.Body.Close() failed", commErrMsg, err)
		}
	}()
	body, err := ioutil.ReadAll(response.Body)
	if err != nil {
		ret.err = err
		fmt.Printf("_HttpClient_processResponse_error||%v||err=%v||read resp body failed", commErrMsg, err)
		return ret, err
	}
	ret.body = body
	...
	if strings.HasPrefix(ret.status, "3") {
		...
		if err == nil && loc != nil {
			...
			ret.err = err
			fmt.Printf("_HttpClient_processResponse_error||%v||err=%v||read resp location failed", commErrMsg, err)
		}
	}
	return ret, nil
}
...
func (ehc *HttpClient) do(httpReq *http.Request) (resp *http.Response, err error) {
	ehc.traceId, _ = strutil.RandomString(32)
	...
	startTime := time.Now()
	defer func() {
		procTime := mathutil.ElapsedTime(startTime)
		...
		//每个http请求都会打印的一条日志信息
		fmt.Printf("_HttpClient_%v||%v||proc_time=%f||code=%v%v", status, ehc.getComErrMsg(), procTime, code, errmsg)
	}()
	...
	for ; retry <= totalRetryTimes; retry++ {
		...
		err = ehc.ctx.Err()
		if err != nil {
			fmt.Printf("_HttpClient_error||%v||msg=ctx.Err||err=%v", ehc.getComErrMsg(), err)
			return
		}
			...
			if totalRetryTimes > 0 {
				...
				if nextBody == nil {
					if body == nil {
						...
						if err != nil {
							fmt.Printf("_HttpClient_error||%v||err=copy httpBody to buffer failed||err=%v", ehc.getComErrMsg(), err)
						}
						body = buf.Bytes()
					}
		...
		if err != nil {
			fmt.Printf("_HttpClient_error||%v||err=fail to send request||err=%v", ehc.getComErrMsg(), err)
			continue
		}
